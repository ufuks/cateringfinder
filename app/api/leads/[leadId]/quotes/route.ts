...
export async function POST(req:Request,{params}:{params:Promise<{leadId:string}>}){
 try{
  ...
  if(!parsed.success)return NextResponse.json({error:'Teklif bilgileri geçersiz.'},{status:400});
 ...
 }catch{return NextResponse.json({error:'Teklif oluşturulamadı.'},{status:400});}
}
